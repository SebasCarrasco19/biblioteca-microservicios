...
private void validateCategory(Long categoryId) {
    try {
        ...
        if (response.getEstado() == null || !"ACTIVO".equalsIgnoreCase(response.getEstado())) {
            throw new RemoteCategoryStateException("La categoría está inactiva");
        }

    } catch (FeignException.NotFound ex) {
        throw new RemoteCategoryNotFoundException(categoryId);

    } catch (RemoteCategoryNotFoundException | RemoteCategoryStateException ex) {
        throw ex;

    } catch (FeignException ex) {
        throw new RemoteServiceException("Error al validar la categoría en category-service");
    }
}
